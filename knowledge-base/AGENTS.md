...
`bootstrap` deploys schema + blueprint, makes the dataset private, enables embeddings, mints a project Viewer token, and imports seed documents. It does not create Knowledge Bases or MCP endpoints. `dev` runs studio (:3333), app (:3000), and functions.
...
- Dataset is **private**. Help-center reads use `SANITY_API_READ_TOKEN` (project Viewer) server-side only.
- Chat uses `SANITY_ORGANIZATION_TOKEN` (Context Viewer) and four `SANITY_MCP_*` URLs. Never send those to the browser.
...
- Seeded contradiction: help article + FAQ = 30-day refunds; `studio/seed/files/customer/beacon-billing-refund-policy.pdf` = 45 days.
