/**
 * J3 — a failed locale is surfaced, not blocking. Mode P.
 */
...
import featureText from './partial-failure.feature?raw'
...
Feature<L10nContext>({
  ...
  stepDefinitions: [
    Given<L10nContext>('a published article', async (context) => {
      context.subject = await publishArticle(harness, {title: 'French fails, German lands'})
    }),
    ...runSteps,
    ...modePSteps,
  ],
