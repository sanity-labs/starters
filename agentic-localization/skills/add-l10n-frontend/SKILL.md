...
| `apps/frontend/src/sanity/live.ts`                     | The client and `defineLive({strict: true})` — `sanityFetch`, `<SanityLive />`, `resolvePreview`, `server-only`                   |
| `apps/frontend/src/sanity/stega.ts`                    | The one field the default stega filter would encode and must not: `l10n.locale.code` keys the fallback graph and every URL       |
...
| `apps/frontend/src/app/[lang]/layout.tsx`              | `<html lang>`, `metadataBase`, `<SanityLive />` and `<VisualEditing />`; `generateStaticParams` from the locale query            |
| `apps/frontend/src/app/api/draft-mode/enable/route.ts` | Where the Studio's Presentation tool enables preview mode — validates its secret, sets the cookies                               |
...
`resolvePreview()` is that read; passing its result as an argument also keys the
cache, so a draft render never lands in the published entry.
