...
| **[ENFORCED]** The floor is enforced three ways: manifests, a lint zone, and a resolved-module-graph assertion over each entry. Prose is not enforcement. | `agentic-localization/oxlint.config.ts` (the `packages/l10n/src/**` override), `packages/l10n/src/exports.test.ts` |
...
| Lint            | DEFAULT  | oxlint extending `@sanity/plugin-kit/oxlint` via `extends` — never copy-paste the preset. Landed in `agentic-localization/oxlint.config.ts`.                                                                                             |
| Engines         | DEFAULT  | `>=22.18`                                                                                                                                                                                                                                |
